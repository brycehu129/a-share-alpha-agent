...
- **`clist/get` 的两个实测约束（2026-09-24 盘中，本地）**：
  ...
  2. **密集请求会被持续封禁，不是偶发抖动。** 实测约 3.5 次/分钟持续几分钟后，https 与 http **同时**返回 502，
     且 `market_review.get_json` 的「重试 1 次 + 降级 http」挡不住；停止请求后约 1–2 分钟才恢复，期间背靠背两轮扫描必被挡。
     所以定时任务要控制密度（抗跌扫描是 3 次/5 分钟 ≈ 0.6 次/分钟）、秒偏移错开，并且**取不到就沿用上一次成功值并标 stale**，不要靠重试硬扛。
