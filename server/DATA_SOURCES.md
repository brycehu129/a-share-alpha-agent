...
| 抗跌扫描的板块**备用源** | 新浪 `MoneyFlow.ssl_bkzj_bk`（48 个行业一次拿全） | 仅在东财板块取不到时；映射表每工作日 09:10 重建（`alpha-shadow-sina-sectors.timer`） | 映射表 3 天过期 |
...
- **`clist/get` 的两个实测约束（2026-09-24 盘中，本地）**：
  ...
  3. **新浪是板块的备用源，但覆盖不全，所以不能当主力**（2026-09-24 实测）：
     新浪 `MoneyFlow.ssl_bkzj_bk` 一次请求就能拿全 48 个行业（涨跌幅 + 主力净流入 + 净流入占比），
     是另一台主机、当天东财封了 25 分钟以上时它零失败，数据也与东财逐个吻合
     （新华传媒 +10.06%、云煤能源 +10.04%、七匹狼 +9.98%）。
     **但它那套行业分类陈旧**：成分股只覆盖 2749 / 5561 ≈ **49%** 的沪深A股，
     其中**科创板一只都没有**，创业板只收 460 只，沪主板也只有约六成
     （48 个板块逐个翻页实测，无失败、无截断）。当主力会把近一半候选静默判成"板块未匹配"。
     另外新浪不给涨跌家数，强度只能两维打分（东财是三维），两者的 strength **不可直接比较**，
     所以每行带 `strength_basis` 标明出处。
     新浪也**不在行情响应里给个股所属行业**，只能按板块列成分股反推，
     因此需要一张每日重建的映射表（`sina_sectors.py --build`，约 100 次请求 / 130 秒）。
  4. **东财一页只回 100 个板块，对"成员判定"不是缺陷。** 抗跌扫描的判据是
     "该股所属板块在主力净流入占比前 100 名内"——排不进前 100 本来就说明板块不够强，
     **截断即过滤**。截断只在需要全量分布（算百分位）时才有害。
  5. **编号域名不存在。** `1./7./23./50.push2.eastmoney.com` 连接直接被关（Remote end closed connection
