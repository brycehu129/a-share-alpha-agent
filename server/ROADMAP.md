...
- **已做**：一字涨停（判据：最高价 == 最低价，全天只有一个价、买不进）与 ST/*ST/退市整理
  默认排除；两者仍留档并带 `is_st` / `one_word_limit` / `limit_up` 标志位，页面可关掉开关。
  「盘中封板」不算一字板——它更早时候能买、也可能炸板，单独标记。

- **⚠️ 欠着的实盘核对 —— 交易时段内跑一条命令即可**：

  ```
  python3 server/resilience_scan.py --verify
  ```

  2026-09-24 上线当天东财把开发机限流封了一下午，以下三件事只做了代码层面的测试、
  **没有在真数据上验过**。`--verify` 把它们固化成逐条 pass/fail/skip，不用靠人记：
  1. **板块匹配率**：`f100`（个股所属行业）与板块清单名字的整体精确匹配率。
     漏斗最后一层全靠它。当天只有"一轮 16/16 命中全匹配"的旁证，不是全量统计。
     低于 90% 会报 fail 并列出对不上的行业名。
  2. **排除真的生效**：一字板与 ST 不得出现在命中列表里。
  3. **服务器的限流表现**是否与开发机一致（ROADMAP 第 12 项：境外机房可能不同）。
  **必须在交易时段跑**：收盘后涨跌幅定格，一字板判据仍有效，但"此刻资金流"和限流表现都不具代表性。

- **未做 / 待定**：
  - 是否升级成 `server/strategies/` 里的一个策略（产生候选、进影子账户），等留档数据说话，本期不做。
  - 阈值固化：跑满 3–5 个交易日后回看 `intraday/resilience-*.jsonl` 的分布再定，
    照第 9 项同一条纪律，不要拍脑袋。
